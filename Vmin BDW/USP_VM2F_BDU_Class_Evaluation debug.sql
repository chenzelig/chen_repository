USE [MPDExploration]
GO
--------------------------Preparation for the SP---------------------------------------------------------

IF OBJECT_ID (N'[MPDExploration]..VM2F_BDU_Class_UT_ClassTests_tempSummary', N'U') IS NOT NULL
	DROP TABLE [MPDExploration]..VM2F_BDU_Class_UT_ClassTests_tempSummary;

select top 0 GroupID,CNT,OS_CNT,OS_10MV,OS_20MV,OS_30MV,OS_40MV,OS_50MV,OS_50PLUS,AVG_STEPS_WP,AVG_STEPS_NP,TTR
into VM2F_BDU_Class_UT_ClassTests_tempSummary
...
DECLARE 
@GroupID int=616
,@groupTable  NVARCHAR(1000)='VM2F_BDU_Class_UT_ClassTests_temp'
,@FlatTable NVARCHAR(1000)='VM2F_BDU_Class_UT_FLat_Table'
...
DECLARE @Equation VARCHAR (MAX),@CMD VARCHAR(MAX),@Test sysname,@SQLString NVARCHAR(MAX),
			 @TEST_IDS VARCHAR(MAX),@T8EST_LIST VARCHAR(MAX), @TEST_LIST2 VARCHAR(MAX),@TEST_LIST3 VARCHAR(MAX),
...
PRINT '********************************************'+char(13)
--*****************************************************************
SET @FailPoint=3 --GROUP LEVL SUMMARY
       

       SET @SQLString = '
    MERGE ' + @groupTable + 'Summary AS target
    USING (
	   SELECT GroupID, E.*,AVG_STEPS_WP,AVG_STEPS_NP,TTR	   	   
