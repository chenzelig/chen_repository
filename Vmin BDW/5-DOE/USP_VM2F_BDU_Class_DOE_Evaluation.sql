...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[USP_VM2F_BDU_Class_DOE_Evaluation]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].[USP_VM2F_BDU_Class_DOE_Evaluation]
...
CREATE PROCEDURE [dbo].[USP_VM2F_BDU_Class_DOE_Evaluation]
...
SET @SQL='
	...
	FROM (
		...
		On ''TESTTIME_''+CT.TestName LIKE TD.testName+''%'' -- Some of the testnames have additional suffix 
	) TT1
	INNER JOIN (
		SELECT 
			 unitID
...
FROM (
	SELECT 
	     ...
	     ,[Steps Without Prediction]= CAST(1.0*SUM(Step_NP)/COUNT(DISTINCT F.UnitID) AS decimal(18,2))
	     ,[Steps With Prediction]=CAST(1.0*SUM(Step_WP)/COUNT(DISTINCT F.UnitID) AS decimal(18,2))
	    
		 -- Per Test Program
	     ,[TT Per Unit Without Prediction]=CAST(SUM(TT_NP)/(1.0*@FilteredUnitCount) AS decimal(18,2))
		 ,[TT Per Unit With Prediction]=CAST(SUM(TT_WP)/(1.0*@FilteredUnitCount) AS decimal(18,2))
		 ,[Steps Per Unit Without Prediction] = CAST(SUM(Step_NP)/(1.0*@FilteredUnitCount) AS decimal(18,2))
	     ,[Steps Per Unit With Prediction] = CAST(SUM(Step_WP)/(1.0*@FilteredUnitCount) AS decimal(18,2))
	FROM #Target T
...
SELECT 
	...
	,[TTR]=CAST(SUM([TT Per Unit Without Prediction]-[TT Per Unit With Prediction])  AS decimal (18,2))
